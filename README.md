...
It exposes readonly `ra_*` MCP tools for hover, definitions, references, document symbols, completions, formatting edits, code actions, `ra_rename_preview`, diagnostics, workspace diagnostics, and workspace switching. Formatting, code action, and rename preview tools return previews only; they do not mutate files.
